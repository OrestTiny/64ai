...
if (!function_exists('upqode_register_nav_menu')) {
	function upqode_register_nav_menu()
	{
		register_nav_menus(array(
			'primary-menu' => esc_html__('Primary Menu', 'upqode'),
		));
	}
}
...
if (!function_exists('upqode_enqueue_scripts')) {
	function upqode_enqueue_scripts()
	{
		...
		if ($blog_page) {
			wp_enqueue_style('upqode-blog-list', UPQODE_T_URI . '/assets/css/blog.min.css');
		}

		if (is_active_sidebar('upqode-sidebar') && $blog_page) {
			wp_enqueue_style('upqode-sidebar', UPQODE_T_URI . '/assets/css/sidebar.min.css');
		}

		if (is_single()) {
			wp_enqueue_style('upqode-blog-single', UPQODE_T_URI . '/assets/css/single.min.css');
		}
